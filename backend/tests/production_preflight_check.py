...
def run_preflight():
    print("=" * 60, flush=True)
    print("RUNNING IP-SAKTI PRODUCTION PREFLIGHT VERIFICATION", flush=True)
    print("=" * 60, flush=True)

    # 1. Config Check
    from src.config import settings
    print(f"[1/6] Config Check:", flush=True)
    print(f"      APP_ENV: {settings.APP_ENV}", flush=True)
    print(f"      BM25_INDEX_PATH: {settings.BM25_INDEX_PATH}", flush=True)
    print(f"      DISABLE_VECTOR_EMBEDDINGS: {settings.DISABLE_VECTOR_EMBEDDINGS}", flush=True)
    print(f"      DISABLE_HEAVY_RERANKER: {settings.DISABLE_HEAVY_RERANKER}", flush=True)
    ...
    print(f"[2/6] BM25 Corpus Check: {corpus_count} chunks loaded.", flush=True)
    ...
    print(f"[3/6] Low-Memory Guarantee Check:", flush=True)
    print(f"      Embedding function: {retriever._embedding_function}", flush=True)
    print(f"      Chroma client: {retriever.chroma_client}", flush=True)
    print(f"      CrossEncoder model: {reranker.model}", flush=True)
    ...
    h_data = h_res.json()
    print(f"[4/6] /health Check: status={h_data.get('status')}, bm25={h_data.get('bm25')}", flush=True)
    ...
    q1_data = q1_res.json()
    print(f"[5/6] Query 'What is a patent?' -> 200 OK | Answer len: {len(q1_data['answer'])}, Citations: {len(q1_data['citations'])}, Confidence: {q1_data['confidence']}", flush=True)
    ...
    q2_data = q2_res.json()
    print(f"[6/6] Query 'Section 3(p)' -> 200 OK | Answer len: {len(q2_data['answer'])}, Citations: {len(q2_data['citations'])}, Confidence: {q2_data['confidence']}", flush=True)
    assert "Section 3(p)" in q2_data["answer"] or len(q2_data["citations"]) > 0
    assert q2_data["human_review"]["recommended"] is True

    print("=" * 60, flush=True)
    print("ALL PRODUCTION PREFLIGHT CHECKS PASSED SUCCESSFULLY!", flush=True)
    print("=" * 60, flush=True)
